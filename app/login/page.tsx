...
export default async function Login({ 
  ...
  async function handleLogin(formData: FormData) {
    ...
    const token = await signToken({ userId: user.id, role: user.role, onboardingCompleted: user.onboardingCompleted, sessionVersion: user.sessionVersion });
          ...
          <form action={handleLogin} className="space-y-6">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <Link href="/forgot-password" className="text-sm text-brand-600 hover:text-brand-500 font-bold transition-colors">Forgot?</Link>
              </div>
              <input 
                type="password" 
                name="password" 
